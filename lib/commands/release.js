...
var validHooks = [
  'beforeCommit' // TODO: add other hooks: afterCommit, beforeTag, afterTag, beforePush, afterPush
];

var ReleaseCommand = Command.extend({
  name: 'release',
  ...
  run: function(cliOptions) {
    ...
    var hooks = this._parsedConfig().hooks;
    ...
    function executeHook(/* hookName, ...args */) {
      ...
      if (hooks[hookName]) {
        return resolve(hooks[hookName].apply(hooks, args));
      } else {
        return resolve();
      }
    }
    ...
    return resolve()
      ...
      .then(function(versions) {
        ...
        return executeHook('beforeCommit', project, versions).then(function(){
          return versions;
        });
      })
      .then(function(versions) {
  ...
  _parsedConfig: function(){
    if (!this._parsedConfigResult) {
      var options = {};
      if (fs.existsSync(path.join(this.project.root, 'config/release.js'))) {
        options = require(path.join(this.project.root, 'config/release'));
      }
      var hooks = extractHooks(options, validHooks);
      this._parsedConfigResult = {
        options: options,
        hooks: hooks
      };
    }
    return this._parsedConfigResult;
  }
});


function extractHooks(obj, validHooks) {
  return validHooks.reduce(function(results, hookName){
    if (typeof obj[hookName] === 'function') {
      results[hookName] = obj[hookName];
      delete obj[hookName];
    }
    return results;
  }, {});
...
Object.defineProperty(ReleaseCommand.prototype, 'availableOptions', {
  get: function(){
    if (!this._availableOptions) {
      var optionsFromConfig = this._parsedConfig().options;
      this._availableOptions = [
        ...
        if ((optionsFromConfig[option.name] !== undefined) && (option.default !== undefined)) {
          ...
          option.description = option.description + ' (configured in release.js)';
        }
        return option;
      });
    }
