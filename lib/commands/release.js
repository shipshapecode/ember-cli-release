  ...
  run: function(cliOptions) {
    ...
    return repo.currentTag().then(function(currentTag) {
      ...
      [ 'package.json', 'bower.json' ].forEach(function(file) {
        command.replaceVersionInFile(file, versions.next);
      });
      ...
      return proceedPrompt(ui, "About to create tag '" + versions.next + "'" + (options.local ? "" : " and push to remote '" + options.remote + "'"))
        .then(function() {
          return versions.next;
        });
