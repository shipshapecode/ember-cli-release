...
module.exports = {
  ...
  run: function(options) {
    ...
    function abortIfCannotPublish() {
      ...
      return npm.whoami([], true).then(function(username) {
        ...
        if (error && error.code === 'ENEEDAUTH') {
          throw new SilentError("Must be logged in to preform NPM publish.");
        }
    ...
    function generateChangelog(tags) {
      ...
      try {
        changelogTask = require('ember-cli-changelog/lib/tasks/changelog');
      } catch (error) {
        // Continue if ember-cli-changelog isn't in the project
        return;
      }
      ...
      return readJSON(filePath, 'utf8').then(function(pkg) {
        // TODO: this doesn't use tags.latest as the starting point, which makes
        // sense I think, but it's inconsistent with what gets passed to the hooks
        // in the case where the package version is not the 'latest', e.g. when
        // releasing from a stable 1.x branch when the latest is 2.x
        // Really, the behavior of this command should be to treat current package
        // version as the 'latest'...
        return changelogTask(project, pkg.version, tags.next);
      });
    }
    ...
    function performPublish() {
      ...
      return readJSON(filePath, 'utf8').then(function(pkg) {
        ...
        ui.writeLine(chalk.green("Publish succesful."));
      });
    }
    ...
    return resolve()
      ...
      .then(function(tags) {
        return executeHook('init', tags)
          ...
          .then(withArgs(generateChangelog, tags.latest))
