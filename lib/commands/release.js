...
var ReleaseCommand = Command.extend({
  ...
  run: function(cliOptions) {
    ...
    function executeHook(hookName /* ...args */) {
      ...
      return function() {
        if (hook) {
          return resolve(hook.apply(hooks, args));
        } else {
          return resolve();
        }
      };
    ...
    return resolve()
      ...
      .then(function(versions) {
        ...
        return executeHook('beforeCommit', versions)()
          ...
          .then(executeHook('afterCommit', versions))
          .then(function() {
            return promptToCreateTag(versions.next);
          })
          .then(executeHook('beforeTag', versions))
          .then(function() {
            return createTag(versions.next);
          })
          .then(executeHook('afterTag', versions))
          .then(executeHook('beforePush', versions))
          .then(pushChanges)
          .then(executeHook('afterPush', versions));
      });
  },
  git: function() {
    if (this._repo) {
...
Object.defineProperty(ReleaseCommand.prototype, 'availableOptions', {
  get: function() {
    if (!this._availableOptions) {
      ...
      this._availableOptions = availableOptions.map(function(availableOption) {
