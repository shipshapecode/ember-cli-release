...
describe("semver strategy", function() {
  ...
  it("should increment the minor version if specified", function() {
    var tag = semverStrategy.getNextTag(tagNames, { minor: true });

    expect(tag.next).to.equal('3.2.0');
  });

  it("should increment the minor version if specified", function() {
    var tag = semverStrategy.getNextTag(tagNames, { major: true });

    expect(tag.next).to.equal('4.0.0');
  });

  it("should throw if tags are present but none are semver compliant", function() {
    expect(semverStrategy.getNextTag.bind(null, [ 'foo' ], {})).to.throw("The repository has no tags that are SemVer compliant, you must specify a tag name with the --tag option.");
  });

  it("should add the 'v' prefix to tags if it's used in the latest tag", function() {
    var tag = semverStrategy.getNextTag([ '0.1.0', 'v0.1.1' ], {});
